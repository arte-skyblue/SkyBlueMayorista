...
const DriftWall = ({
  ...
  const renderTile = (item, id, colIndex) => {
    const inner = (
      <span className="drift-wall__inner group/tile cursor-pointer">
        <img src={item.image} alt={item.title ?? ''} loading="lazy" decoding="async" draggable={false} />
        <span className="drift-wall__overlay" aria-hidden="true" />
        {item.title && (
          <span className="absolute bottom-0 inset-x-0 p-3 bg-gradient-to-t from-black/90 via-black/40 to-transparent text-white text-xs font-bold opacity-0 group-hover/tile:opacity-100 transition-opacity duration-300 pointer-events-none flex flex-col gap-0.5">
            {item.brand && (
              <span className="text-[10px] text-amber-300 uppercase font-black">{item.brand}</span>
            )}
